...
for SNRi = info.SNRvals
  ...
  for idxRand = 0:info.nTrials
    if idxRand == 0
      ...
      result.nPatches = randsample(info.minPatchs:info.maxPatchs, 1);
      result.idxCent      = zeros(result.nPatches,1);
      result.IntendedCent = zeros(result.nPatches,3);
      for idxPatch = 1:result.nPatches
        found_source = false;
        while ~found_source
          tmp = randi([1 meta.nGridDips]);
          % 1: source must be within given distance to scalp
          if meta.GridDepth(tmp) < info.maxDepth
            minDist = Inf;
            proposedCent = meta.Gridloc(tmp,:);
            for idxPatch_pre = 1:(idxPatch-1)
              currDist = norm( result.IntendedCent(idxPatch_pre,:) - proposedCent, 2 );
              if currDist < minDist
                minDist = currDist;
              end
            end
            if minDist > info.distPatch
              found_source = true;
              result.idxCent(idxPatch) = tmp;
              result.IntendedCent(idxPatch,:) = meta.Gridloc(tmp,:);
            end
          end
        end
      end
    ...
    if info.minKappa == info.maxKappa
      result.kappa = ones(result.nPatches,1)*info.minKappa;
    else
      result.kappa = info.minKappa + rand(result.nPatches,1)*(info.maxKappa-info.minKappa);
    end
    %
